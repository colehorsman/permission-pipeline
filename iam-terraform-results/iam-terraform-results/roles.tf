resource "aws_iam_role" "AirIAM-AccessAnalyzer-Role" {
  name        = "AirIAM-AccessAnalyzer-Role"
  path        = "/"
  description = ""

  assume_role_policy = data.aws_iam_policy_document.AirIAM-AccessAnalyzer-Role_assume_role_policy_document.json
...
data "aws_iam_policy_document" "AirIAM-AccessAnalyzer-Role_assume_role_policy_document" {
  version = "2012-10-17"
  statement {
    effect  = "Allow"
...
resource "aws_iam_role_policy" "AirIAM-AccessAnalyzer-Role_AirIAM-AccessAnalyzer-Policy" {
  name   = "AirIAM-AccessAnalyzer-Policy"
  policy = data.aws_iam_policy_document.AirIAM-AccessAnalyzer-Role_AirIAM-AccessAnalyzer-Policy_document_document.json
  role   = aws_iam_role.AirIAM-AccessAnalyzer-Role.name
}

data "aws_iam_policy_document" "AirIAM-AccessAnalyzer-Role_AirIAM-AccessAnalyzer-Policy_document_document" {
  version = "2012-10-17"
  statement {
    effect    = "Allow"
    actions   = ["cloudtrail:LookupEvents", "cloudtrail:GetTrailStatus", "cloudtrail:DescribeTrails", "cloudtrail:GetTrail", "iam:Get*", "iam:List*"]
    resources = ["*"]
